...
def test_multi_sentence_pdf_task_success(mock_extract, mock_tts, mock_redis):
    ...
    mock_extract.assert_called_once_with(fake_pdf_bytes)
    
    assert mock_tts.call_count == 2
    assert mock_redis.publish.call_count == 3 # 3 since checking for "__COMPLETE"

    mock_redis.publish.assert_any_call(f"audio_stream:{fake_job_id}", b"\x00\x01")
    mock_redis.publish.assert_any_call(f"audio_stream:{fake_job_id}", b"\x02\x03")
    mock_redis.publish.assert_any_call(f"audio_stream:{fake_job_id}", b"__COMPLETE__")